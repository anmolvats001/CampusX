...
const IssueNavBar = () => {
    ...
    return(
        <div className={`${dark ? "text-white" : "text-black"} h-16 w-[45%] px-8 backdrop-blur-xl fixed border border-gray-800 z-99`}>
            <div className="flex issues-nav justify-between items-center  h-full cursor-pointer ">
                ...
                <div onClick={()=>{setVal("in-process")}}className={"hover:text-lg transition-all capitalize duration-300 ease-in-out "+(val=="in-process"&&"border-b-4 border-b-white  bg-gray-700 px-3 rounded-lg py-1 transition-all duration-300 ease-in-out ")+(!dark&&val=="in-process" &&" border-b-4 border-b-black text-white transition-all duration-300 ease-in-out")}>in process</div>
